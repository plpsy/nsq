...
var (
	...
	sliceSize = flag.Int("sliceSize", 4096, "slice size send to server")
	...
	go func() {
		for {
			...
			if throttleEnabled {
				...
				err = readAndPublish(r, delim, *sliceSize, producers)
...
func readAndPublish(r *bufio.Reader, delim byte, lineBytes int, producers map[string]*nsq.Producer) error {
	var line []byte
	if lineBytes == 0 {
